...
// TEST0083: Compose timeout and logging wrappers around a failing op and verify the error message includes the op name
#[tokio::test]
async fn test0083_error_handling_and_wrapper_chains() {
...
// TEST0084: Call get_caller_trigger_name from within a test and verify it reflects the integration test module path
#[tokio::test]
async fn test0084_stack_trace_analysis() {
    let trigger_name = get_caller_trigger_name();
    assert!(trigger_name.contains("integration_tests"));
    assert!(trigger_name.contains("::"));
}

// TEST0085: Use wrap_nested_op_exception and verify the wrapped error contains both op name and original message
#[tokio::test]
async fn test0085_exception_wrapping_utilities() {
...
// TEST0086: Wrap a slow op in a short-timeout TimeBoundWrapper and verify the error is wrapped with logging context
#[tokio::test]
async fn test0086_timeout_wrapper_functionality() {
    tracing_subscriber::fmt::try_init().ok();
    let mut dry = DryContext::new();
    let mut wet = WetContext::new();
...
// TEST0087: Run an op that retrieves a service from WetContext and reads config values from it
#[tokio::test]
async fn test0087_dry_and_wet_context_usage() {
    let mut dry = DryContext::new();
...
// TEST0088: Run a BatchOp with two identical user-building ops and verify both produce the expected User struct
#[tokio::test]
async fn test0088_batch_ops() {
    let mut dry = DryContext::new()
...
// TEST0089: Compose TimeBoundWrapper and LoggingWrapper around a simple op and verify the result passes through
#[tokio::test]
async fn test0089_wrapper_composition() {
    let mut dry = DryContext::new();
    let mut wet = WetContext::new();
...
// TEST0090: Use the perform() utility function directly and verify it returns the op result with auto-logging
#[tokio::test]
async fn test0090_perform_utility() {
    let mut dry = DryContext::new();
    let mut wet = WetContext::new();
